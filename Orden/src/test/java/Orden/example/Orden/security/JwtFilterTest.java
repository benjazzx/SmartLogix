...
class JwtFilterTest {
    ...
    void tokenValido_conUserId_autenticaYSetAttribute() throws Exception {
        String token = "token.orden.valido";
        UUID userId = UUID.randomUUID();
        when(jwtUtil.isTokenValid(token)).thenReturn(true);
        when(jwtUtil.extractUsername(token)).thenReturn("admin@orden.cl");
        ...
        assertNotNull(auth);
        assertEquals("admin@orden.cl", auth.getPrincipal());
    ...
    void tokenValido_sinUserId_autenticaSinAttribute() throws Exception {
        ...
        when(jwtUtil.isTokenValid(token)).thenReturn(true);
        when(jwtUtil.extractUsername(token)).thenReturn("u@orden.cl");

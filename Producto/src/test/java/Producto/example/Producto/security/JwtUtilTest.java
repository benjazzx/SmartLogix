...
class JwtUtilTest {
    ...
    void extractAllClaims_tokenValido_retornaClaims() {
        String token = buildToken("admin@prod.cl", "admin", UUID.randomUUID(), 3_600_000);
        Claims claims = jwtUtil.extractAllClaims(token);
        assertNotNull(claims);
        assertEquals("admin@prod.cl", claims.getSubject());
    }

    @Test
    void extractUsername_retornaCorreo() {
        String token = buildToken("bodeguero@prod.cl", "bodeguero", null, 3_600_000);
        assertEquals("bodeguero@prod.cl", jwtUtil.extractUsername(token));
    }

    @Test
    void extractRol_retornaRolNombre() {
        String token = buildToken("x@prod.cl", "transportista", null, 3_600_000);
    ...
    void extractUserId_conUserId_retornaUUID() {
        ...
        String token = buildToken("u@prod.cl", "admin", userId, 3_600_000);
        assertEquals(userId, jwtUtil.extractUserId(token));
    }

    @Test
    void extractUserId_sinUserId_retornaNull() {
        String token = buildToken("u@prod.cl", "admin", null, 3_600_000);
        assertNull(jwtUtil.extractUserId(token));
    }

    @Test
    void isTokenValid_vigente_retornaTrue() {
        assertTrue(jwtUtil.isTokenValid(buildToken("ok@prod.cl", "admin", null, 3_600_000)));
    }

    @Test
    void isTokenValid_expirado_retornaFalse() {
        SecretKey key = Keys.hmacShaKeyFor(SECRET.getBytes(StandardCharsets.UTF_8));
        String token = Jwts.builder().subject("exp@prod.cl")

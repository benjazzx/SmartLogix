...
@Tag(name = "Usuarios", description = "Gestión de usuarios del sistema SmartLogix")
public class UserController {
    ...
    public ResponseEntity<UserModel> getUserByCorreo(
            @Parameter(description = "Correo electrónico del usuario") @PathVariable String correo) {
        try {
            return ResponseEntity.ok(userService.getUserByCorreo(correo));
        } catch (RuntimeException e) {
            return ResponseEntity.notFound().build();
        }
    }
    ...
    public List<UserModel> getUsersByRol(
    ...
    @PostMapping
    public ResponseEntity<UserModel> createUser(@RequestBody UserRequestDto dto) {
        try {
    ...
    public ResponseEntity<UserModel> asignarRol(
            @PathVariable UUID id,
            @RequestBody Map<String, String> body) {
        try {
            UUID rolId = UUID.fromString(body.get("rolId"));
            String rolNombre = body.get("rolNombre");
            return ResponseEntity.ok(userService.asignarRol(id, rolId, rolNombre));
        } catch (RuntimeException e) {
            return ResponseEntity.notFound().build();
        }
    }

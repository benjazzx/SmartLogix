...
class JwtFilterTest {
    ...
    void tokenValido_conUserId_autenticaYSetAttribute() throws Exception {
        ...
        when(jwtUtil.extractUsername(token)).thenReturn("bodeguero@test.cl");
        ...
        assertNotNull(auth);
        assertEquals("bodeguero@test.cl", auth.getPrincipal());
    ...
    void tokenValido_sinUserId_autenticaSinAttribute() throws Exception {
        ...
        when(jwtUtil.isTokenValid(token)).thenReturn(true);
        when(jwtUtil.extractUsername(token)).thenReturn("admin@test.cl");
